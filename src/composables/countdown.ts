...
export function useDateCountdown() {
  ...
  const countdown = computed(() => {
    console.log(currentDate.value, countdownDate.value);
    return getDateDifferenceObject(currentDate.value, countdownDate.value);
  });
  ...
  const compareDates = () => {
    ...
    const diffInMs = getDateDifferenceMs(nowDate, countdownDate.value);
    console.log("COMPARA");
    if (diffInMs <= 0) {
      clearInterval(timeInterval.value);
  ...
  const resumeCountdown = () => {
    ...
    const diffInMs = getDateDifferenceMs(countdownDate.value, currentNewDate);
   ...
   */
  const startCountdown = (date: Date, updateInterval: number = 1000) => {
  ...
  return {
    miliseconds: computed(() => countdown.value.milliseconds),
    ...
    months: computed(() => countdown.value.months),
    years: computed(() => countdown.value.years),
